...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // request, deleted on accept or expiry).
  ...
  if (resetRow.expiresAt < new Date()) {
    // Prune stale row and surface a distinct expiry error so the UI
    // can show "this link expired, please request a new one" instead
    // of the generic "invalid" message.
    await prisma.verification.delete({ where: { id: resetRow.id } }).catch(() => {});
    return NextResponse.json({ ok: false, error: "This reset link has expired" }, { status: 400 });
  }
  ...
  if (!user) {
    return NextResponse.json({ ok: false, error: "Account unavailable" }, { status: 400 });
  }
